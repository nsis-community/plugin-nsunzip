...
/*
    ...
    neflag            = replace only existing files
...
LPDCL lpDCL;
{
    ...
    uO.uflag = lpDCL->ExtractOnlyNewer || (!lpDCL->neflag && lpDCL->nfflag);
    uO.fflag = (lpDCL->neflag || lpDCL->nfflag);
#ifdef WIN32
    ...
    ZCONST char *efn;   /* name of archiv entry being processed */
{
    ...
    return (*G.lpUserFunctions->password)((LPSTR)pwbuf, size, zfn/*m*/, (LPSTR)efn);
#else /* !CRYPT */
    ...
    switch (fnflag) {
      ...
      case UZ_ST_FINISH_MEMBER:
        if ((G.lpUserFunctions->ServCallBk != NULL) &&
            (*G.lpUserFunctions->ServCallBk)(G.zipfn,
                                             G.ziplen,
                                             efn,
                                             G.lrec.ucsize,
                                             G.lrec.csize,
